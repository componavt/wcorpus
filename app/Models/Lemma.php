...
//        $builder = $builder -> orderBy('lemma');
    ...
    public static function countByIDAndAuthor($id,$author_id) {
        ...
        $results = DB::select( DB::raw("SELECT count(*) as count FROM sentence_wordform "
                 . "WHERE sentence_id in (SELECT id FROM sentences WHERE "
                . "text_id in (select id FROM texts where author_id=".(int)$author_id.")) "
                 . "AND wordform_id in (select wordform_id from lemma_wordform WHERE lemma_id=".(int)$id.")") );
        if (!$results) {
            return null;
        }
